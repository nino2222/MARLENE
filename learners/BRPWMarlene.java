...
public class BRPWMarlene {
    ...
    public IntOption randomSeed = new IntOption("numberOfLabels", 'r',
            "Random Seed", 1, 1, Integer.MAX_VALUE);
    ...
    public DLearner generateDCLF(){
        ...
        dc.setRandomSeed(this.randomSeed.getValue());
        dc.prepareForUse();
        dc.resetLearning();
        return dc;
    }
        ...
        for (int i = 0; i < labels.length; i++){
            ...
            for (int j = 0; j < labels.length; j++){
                if (i != j){
                    ...
                    //Detect Drift
                    if (manualSetConceptDriftOption.isSet()){
                        if (reset && (ArrayUtils.contains(numberOfReset, i) || ArrayUtils.contains(numberOfReset, j))) {
//                            System.out.println("i: " + i + " j: " + j);
                            this.resetAllPerfOnDependence(i, j);
                            dclf_lists[i][j].add(this.generateDCLF());
                        }
                    }else {
...
//                                        }
    ...
    public double[][] getWeightRatio(){
        double[][] wr = new double[this.numOfLabels.getValue()][this.numOfLabels.getValue()];
        ...
        for (int i = 0; i < this.numOfLabels.getValue(); i++){
            for (int j = 0; j < this.numOfLabels.getValue(); j++){
                if (j != i){
                    ...
                    for (List<DLearner>[][] dc_lists: d_clfs.values()){
                        for (int si = 0; si < dc_lists.length; si++) {
                            for (int sj = 0; sj < dc_lists[si].length; sj++) {
                                if (si != sj) {
                                    for (DLearner dc : dc_lists[si][sj]) {
                                        wr[i][j] += dc.getDPerfs()[i][j].getA();
                                        if (dc_lists.equals(d_clfs.get(this.targetDomainIndexOption.getValue()))
                                                && si == i && sj == j
                                        ) {
                                            cw += dc.getDPerfs()[i][j].getA();
                                        }
                                    }
                                }
                            }
                    ...
                    if (Double.isNaN(wr[i][j])){
                        wr[i][j] = 0;
                    }

                    wr[i][j] = wr[i][j] == 0 ? 0 : (wr[i][j] - cw)/wr[i][j];
                }
            }
        }
    ...
    public double getASWR(){
        ...
        double[] twr = new double[this.numOfLabels.getValue()];
        for (int i = 0; i < wr.length; i++){
            twr[i] = Utils.sum(wr[i]);
        }

//        System.out.println(Utils.sum(twr)/(wr.length*(wr.length-1)));
        return Utils.sum(twr)/(wr.length*(wr.length-1));
    }
