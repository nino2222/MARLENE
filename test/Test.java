...
//        String txt_name = "Yeast_labels.txt";
        ...
        String larff_name = "Case7_500/Case7_500_l.arff";
        String darff_name = "Case7_500/Case7_500_d.arff";
        String txt_name = "Case7_500/Case7_500_labels.txt";
...
//        }
        ...
        BufferedWriter writerPR = new BufferedWriter(new FileWriter(new File("/home/h/hd168/MarleneOutput/Performance/" +
                "PreResults.csv")));
...
//            System.out.println("aswr: " + aswr);
            writerPR.write(l_insts[i].valueInputAttribute(0) + ",");
            writerPR.write(l_insts[i].valueInputAttribute(1) + ",");
            writerPR.write(Utils.maxIndex(pred.getVotes(0)) + "\n");
...
//            }
        ...
        Instance[] dBoundaryl_insts = utils.readInstances("/home/h/hd168/MarleneDataSet/DBData/DBTestData_l.arff");
        Instance[] dBoundaryd_insts = utils.readInstances("/home/h/hd168/MarleneDataSet/DBData/DBTestData_d.arff");

        BufferedWriter writerTD = new BufferedWriter(new FileWriter(new File("/home/h/hd168/MarleneOutput/Performance/" +
                "DecisionBoundaryTD.csv")));

        for (int i = 0; i < l_insts.length; i++) {
            writerTD.write(l_insts[i].valueInputAttribute(0) + ",");
            writerTD.write(l_insts[i].valueInputAttribute(1) + ",");
            writerTD.write(labels[i][0]+ "\n");
        }

        writerTD.close();

        BufferedWriter writerR = new BufferedWriter(new FileWriter(new File("/home/h/hd168/MarleneOutput/Performance/" +
                "DecisionBoundaryR.csv")));

        for (int i = 0; i < dBoundaryl_insts.length; i++){
            Prediction dBpred = marlene.getPredictionForInstance(dBoundaryl_insts[i], dBoundaryd_insts[i]);
            System.out.println(dBpred);
            writerR.write(dBoundaryl_insts[i].valueInputAttribute(0) + ",");
            writerR.write(dBoundaryl_insts[i].valueInputAttribute(1) + ",");
            writerR.write(Utils.maxIndex(dBpred.getVotes(0)) + "\n");
        }

//        for (int i = 0; i < l_insts.length; i++){
//            Prediction dBpred = marlene.getPredictionForInstance(l_insts[i], d_insts[i]);
//            System.out.println(dBpred);
//            writerR.write(l_insts[i].valueInputAttribute(0) + ",");
//            writerR.write(l_insts[i].valueInputAttribute(1) + ",");
//            writerR.write(Utils.maxIndex(dBpred.getVotes(0)) + "\n");
//        }
